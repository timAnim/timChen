(function(window) {
    var util = {
        ...
        selectImg: function(type) {
            ...
            document.body.appendChild(pd.toHTML(str));
            pd.id('imgUpload').addEvent('change', function() {
                pd.util.uploadImg(type);
            }, false);
            pd.util.fireClick(pd.id('imgUpload'));
        },
            ...
            var modelId, xhr,
                beforeUpload = function() {
                    ...
                    switch (modelType) {
                        ...
                        case 'MindContentImage':
                            ...
                            break;
                        case 'MindSetImage':
                            modelId = pd.submitObj.setId;
                            break;
                    }
                    var file = pd.id('imgUpload').files[0],
                ...
                beginUpload = function(file) {
                    ...
                    var callback = function(base64) {
                        ...
                        xhr.open("POST", pd.apiUrl + "/image/upload");
                        xhr.send(fd);
                    };
                    pd.util.getBase64Img(file, square, callback);
                },
                uploadResposed = function() {
                    if (xhr.readyState == 4 && xhr.status == 200) {
                        ...
                        if (data.error === 'NA') {
                            switch (modelType) {
                                case 'MindUserImage':
                                    resImg = pd.id('portrait');
                                    url = pd.getIMG(img, resImg.clientWidth);
                                    userInit.portrait = url;
                                    pd.ajax({ url: '/user/flush' });
                                    break;
                                case 'MindContentImage':
                                    var width = pd.tag('title', wvFile.actChild).clientWidth;
                                    url = pd.getIMG(img, width);
                                    wvFile.actChild.parentNode.setAttribute('image', url);
                                    resImg = pd.tag('img', wvFile.actChild);
                                    if (!resImg) {
                                ...
                                case 'MindSetImage':
                                    if (pd.id('coverPt')) {
                                        resImg = pd.id('coverPt');
                                    }
                                    if (pd.id(pd.submitObj.setGuid)) {
                                        resImg = pd.id(pd.submitObj.setGuid);
                                    }
                                    url = pd.getIMG(img, resImg.clientWidth);
                                    break;
                            }
                            pd.util.loadImg(url, function() {
        ...
        getBase64Img: function(file, square, callback) {
            if (square) {
                ...
                reader.onload = function(e) {
                    callback(pd.target(e).result.replace(/^data:image\/(png|jpg);base64,/, ""));
                }
            }
        },
        wechatImg: function(img) {
            var thumb = new Image();
            thumb.src = pd.getIMG(img, 300);
            pd.id('wx_pic').innerHTML = '';
            pd.id('wx_pic').appendChild(thumb);
        },
        ...
        shareTo: function(url, picurl) {
            ...
            var html =
                ...
                "</ul>" +
                "</dialog>";
            pd.append(pd.toHTML(html), document.body);
            pd.log.in('shareDlg');
            (new Hammer(pd.id('shareDlg'), [Hammer.Tap])).on("tap", evHandler);

            function evHandler(ev) {
                var target = pd.target(ev);
                switch (target.id) {
                    case 'shareDlg':
                        pd.log.out('shareDlg');
                        break;
                    ...
                    case 'facebook':
                        if (typeof(FB) != 'undefined') {
                            ...
                            pd.toast(lan.t('Cannot connect Facebook'))
                        }
                        break;
                    default:
                        return;
        ...
        ajax: function(opt) {
            ...
            for (var key in opt.data) {
                if (pd.util.isArray(opt.data[key])) {
        ...
        insert: function(dom, parent, clear) {
            if (clear && pd.id(parent)) {
                if (typeof(parent) === "string") {
                    pd.id(parent).innerHTML = '';
                    pd.id(parent).insertBefore(dom, parent.children[0]);
                } else {
                    parent.innerHTML = '';
        ...
        getIMG: function(img, width) {
            ...
            return pd.apiUrl + '/image/view/' + img.guid + '_' + width + '.' + img.extension;
        },
